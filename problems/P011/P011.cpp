...
int main() {
  ...
  int i, j, k;
  long long largest_prod = 0;
  long long prod1, prod2, prod3, prod4;
  for (i = 0; i < 17; i++) {
    for (j = 0; j < 17; j++) {
      ...
      for (k = 0; k < 4; k++) {
        prod1 *= numbers[i + k][j];
        prod2 *= numbers[i][j + k];
        prod3 *= numbers[i + k][j + k];
        prod4 *= numbers[i + 3 - k][j + k];
      }

      if (prod1 > largest_prod)
        largest_prod = prod1;
      if (prod2 > largest_prod)
        largest_prod = prod2;
      if (prod3 > largest_prod)
        largest_prod = prod3;
      if (prod4 > largest_prod)
        largest_prod = prod4;
    }
  }

  std::cout << largest_prod << std::endl;
